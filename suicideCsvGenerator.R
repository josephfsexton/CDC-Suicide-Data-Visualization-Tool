...
all_suicides$age = as.integer(all_suicides$age) - 1000

new <- subset(suicides, select=c('education', 'month.of.death', 'sex', 'age', 'place.of.death',
                                  'marital.status', 'day.of.week', 'death.year', 'icd.code', 'race_cat',
                                  'hispanic_cat'))

colnames(new) <- c("Education", "Month of Death", "Sex", "Age", "Place of Death",
                   "Marital Status", "Day of Week", "Year of Death", "Means", "Race",
                   "Ethnicity")


?write.csv
write.csv(new, "C:\\rProjects\\suicide\\suicides2.0\\all_suicides.csv")
